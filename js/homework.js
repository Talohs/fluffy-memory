...
async function printMovieInfo(film){
    try {
        let title = await getMovieInfo(film)
        console.log(`${title.title} directed by ${title.director}. A story of ${title.description} that runs for ${title.runtime} minutes.`)
    } catch(err) {
        console.warn(err)
    }
}
